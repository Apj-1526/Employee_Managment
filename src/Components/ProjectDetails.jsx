...
const ProjectDetails = () => {
  ...
  const [newProject, setNewProject] = useState({
    ...
    technicalLead: '',
    status: 'UNKNOWN',
    deadlineDate: '' // Add deadlineDate field
  });
  ...
  const handleEmployeeDelete = async (employeeId, projectId) => {
    if (window.confirm('Are you sure you want to remove this employee from the project?')) {
      try {
        ...
        const response = await axios.get(`http://localhost:9000/api/projectassignments/${projectId}`);
...
const handleCloseModal = () => {
  ...
  <div className="modal-overlay" onClick={closeModal}>
    <div className="modal-card" onClick={e => e.stopPropagation()}>
      ...
      <p><strong>Deadline Date:</strong> {selectedProject.deadlineDate}</p>
...
{selectedProject.assignedEmployees && selectedProject.assignedEmployees.length > 0 ? (
  selectedProject.assignedEmployees.map(employee => (
    <li key={employee.id}>
      ...
      >
        <i className="fi fi-rs-trash text-2xl text-red-500 cursor-pointer"></i>
      </span>
    </li>
  ))
) : (
        ...
        <div className="modal-overlay" onClick={closeAddModal}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <form>
              ...
              <input type="date" value={newProject.deadlineDate} onChange={(e) => handleNewFieldChange('deadlineDate', e.target.value)} />
            </form>
